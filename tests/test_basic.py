...
def test_add_storage(rook_cluster):
    ...
    logger.info("Creating a new disk and attaching to first node")
    ...
    logger.info("Current OSD's:")
    logger.info(osds)
    logger.info("Expected OSD's:")
    logger.info(osds_expected)
    ...
    while osds < osds_expected:
        if i == 120:
            pytest.fail(
                f"rook was not able to add {new_osds} of required osds")
            break
        time.sleep(10)
        osds = rook_cluster.get_number_of_osds()
        i += 1
